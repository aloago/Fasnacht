...
class PictureGridApp:
    def __init__(self, root, image_dir, banner_path, back_button_path, selections_dir, file_names, labels, scaling_factor=1.0, loading_gif_path=None):
        ...
        self.loading_time = 3000  # Duration of the loading screen in milliseconds
    ...
    def gpio_setup(self):
        """Set up GPIO buttons for Raspberry Pi."""
        GPIO.setmode(GPIO.BCM)  # Use BCM pin numbering
        self.freeze_button_pin = 17  # GPIO pin for the freeze button
        self.random_button_pin = 27  # GPIO pin for the random selection button

        # Set up GPIO pins as inputs with pull-up resistors
        GPIO.setup(self.freeze_button_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.setup(self.random_button_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)

        # Add event detection for GPIO buttons
        GPIO.add_event_detect(self.freeze_button_pin, GPIO.FALLING, callback=self.freeze_selection_screen, bouncetime=300)
        GPIO.add_event_detect(self.random_button_pin, GPIO.FALLING, callback=self.random_selection_screen, bouncetime=300)

    def freeze_selection_screen(self, channel):
        """Freeze the current selection screen by removing the back button."""
        if hasattr(self, "back_button"):
            self.back_button.destroy()  # Remove the back button

    def random_selection_screen(self, channel):
        """Randomly select two new images and show their respective selection screen."""
        if len(self.file_names) >= 2:
            # Randomly select two unique images
            random_images = random.sample(self.file_names, 2)
            self.clicked_images = random_images
    ...
    def show_selection_screen(self):
        ...
        if len(sorted_clicked_images) == 2:
            ...
            self.back_button = ttk.Button(self.root, text="← Back", style="Flat.TButton", command=self.reset_selection)
