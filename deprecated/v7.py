...
class PictureGridApp:
    def __init__(self, root, image_dir, banner_path, back_button_path, selections_dir, file_names, labels, scaling_factor=1.0, loading_gif_path=None, background_path=None):
        ...
        self.background_path = background_path  # New parameter for the background image path
        ...
        if self.background_path and os.path.exists(self.background_path):
            ...
            self.background_image = self.background_image.resize((self.root.winfo_width(), self.root.winfo_height()), Image.Resampling.LANCZOS)
            self.background_photo = ImageTk.PhotoImage(self.background_image)
            self.background_canvas = Canvas(self.root)
...
if __name__ == "__main__":
    ...
    root = Tk()
    app = PictureGridApp(root, IMAGE_DIR, BANNER_PATH, BACK_BUTTON_PATH, SELECTIONS_DIR, FILE_NAMES, LABELS, scaling_factor=0.6, loading_gif_path=LOADING_GIF_PATH, background_path=BACKGROUND_PATH)  # Adjust scaling_factor as needed
    root.mainloop()